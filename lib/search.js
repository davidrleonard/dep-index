 ...
 */
search._searchForDep = function(depName) {
  this._log(`Starting search for ${chalk.bold(depName)}...`, 'silly');
  ...
  return this._fetchListOfProjects(this._projectsDirPath, this._whitelist)
    ...
    .then((rawDeps) => {
      ...
      this._log(chalk.yellow(`Found ${foundDeps.length} projects that use ${depName}.`), 'silly');
...
search._fetchListOfProjects = function(projectsDir, whitelist) {
  whitelist = whitelist || [];
  this._log(`Searching the directory at ${chalk.bold(projectsDir)} for projects...`, 'silly');
...
search._findDepInProjects = function(projects, projectsDir, depName) {
  ...
  this._log(`Searching ${chalk.bold(projects.length)} projects for ${chalk.bold(depName)}...`, 'silly');

  let progress = this._tick(`Searching for ${depName}`, projects.length);

  return Promise.all(
    projects.map((project) => {
      progress.increment();
      return this._checkProjectForDep(`${projectsDir}/${project}`, project, depName);
    })
  );
};
...
search._checkProjectForDep = function(projectPath, projectName, depName) {
  return new Promise((resolve, reject) => {
    ...
    let bowerPackage;
    try {
      bowerPackage = require(`${projectPath}/bower.json`);
    } catch (e) {
      if (e.code !== 'MODULE_NOT_FOUND') {
        reject(new Error(`Could not find bower.json file in module: ${projectPath}`));
      }
    }
    const found = [];
    ...
    // For "*", add everything
    if (depName === '*' && bowerPackage && bowerPackage.hasOwnProperty('dependencies')) {
      for (let dep in bowerPackage.dependencies) {
        found.push([projectName, dep, bowerPackage.dependencies[dep]]);
      }
    }

    if (depName === '*' && bowerPackage && bowerPackage.hasOwnProperty('devDependencies')) {
      for (let dep in bowerPackage.devDependencies) {
