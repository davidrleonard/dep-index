...
crawl._crawl = function() {
  this._log(`Starting to crawl dependencies in ${chalk.bold(this._projectsDirPath)}...`, 'info');
...
crawl._crawlAllProjects = function(projects, projectsDir) {
  ...
  this._log(`Crawling ${chalk.bold(projects.length)} projects...`, 'info');
  ...
  return Promise.mapSeries(projects, (project) => {
    current++;
    this._log(`[${current}/${total}] Extracting data from ${chalk.bold(project)}...`, 'silly');
