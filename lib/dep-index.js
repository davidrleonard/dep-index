...
const DepGalaxy = require('./dep-galaxy/dep-galaxy');
...
class DepIndex {
  ...
  /**
   * Determines if there are any global dependency conflicts across many projects
   * in the `projectsDirPath`. Useful to determine if consumers of multiple
   * modules you publish may face issues creating a flat dependency tree. Only
   * analyzes a
   *
   * @method findShallowGlobalConflicts
   * @return {Promise|Object} - An object with information about the dependency
   */
  findShallowGlobalConflicts(findConflicts) {
    // Create a new DepIndex to search through all projects for shallow dependencies
    // (A shallow dependency is a direct dependency of one of the projects in our
    // directory. Deep dependencies are cascading dependencies of each dependency.)
    const searchAllDeps = createDepIndex({
      projectsDirPath: this._projectsDirPath,
      logLevel: 'silent'
    });
    ...
    const depGalaxy = DepGalaxy();



    return searchAllDeps.search('*')
      .then((shallowDeps) => {
        return this._prepareDeepDeps(shallowDeps);
      })
      .then((allDeps) => {
