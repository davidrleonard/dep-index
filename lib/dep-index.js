...
const incompatibles = require('./incompatibles');
...
function createDepIndex(opts) {
  ...
  mixin(depIndex, incompatibles, false);
...
class DepIndex {
  ...
  constructor(opts) {
    ...
    this._prerelease = opts.prerelease || false;
  };
  ...
  findIncompatibles() {
    return this._crawl()
      .then(graph => this._findIncompatibles(graph));
  };
  ...
  _tick(name, total) {
    const MAX_LEN = 30;
    let padded = pad(name, 30);
    return tick(padded, total);
  };
};

function tick(name, total) {
  let counter = new prog.Bar({
    format: `${name} [{bar}] {percentage}% | ETA: {eta}s | {value}/{total}`,
    stopOnComplete: true
  }, prog.Presets.shades_classic);
  let increment = tickUp.bind(this, counter);
  counter.start(total, 0);
  return {counter,increment};
}

function tickUp(counter, val) {
